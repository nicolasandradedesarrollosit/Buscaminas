...
var activarRanking = document.getElementById("activarRanking");
var modalRanking = document.getElementById("modalRanking");

iniciarJuego.disabled = true;

var victoria = new Audio("public/ganado.wav");
var derrota = new Audio("public/perdido.mp3");
...
function generarTablero(filas, columnas, minas) {
  contenido.innerHTML = "";
  contenido.style.width = `${columnas * 42}px`;
  contenido.style.minHeight = filas * 40 + (filas - 1) * 2 + "px";
  contenido.style.height = "fit-content";

  var arrayBombas = Array(minas).fill("bomba");
  var arrayVacios = Array(filas * columnas - minas).fill("vacio");
  var arrayCompleto = arrayVacios.concat(arrayBombas).sort(function () {
...
function click(casilla) {
  ...
  if (
    casilla.classList.contains("revelada") ||
  ...
  if (minasAlrededor > 0) {
    ...
    casilla.style.color =
      minasAlrededor === 1 ? "blue" : minasAlrededor === 2 ? "green" : "red";
  } else {
    casilla.textContent = "";
...
function dobleClick(casilla) {
  ...
  for (var i = 0; i < vecinos.length; i++) {
    if (juego.casillas[vecinos[i]].classList.contains("bandera"))
      banderasCerca++;
  }
  ...
  if (banderasCerca === numeroActual) {
    for (var j = 0; j < vecinos.length; j++) {
      ...
      if (
        !vecino.classList.contains("bandera") &&
        !vecino.classList.contains("revelada")
      ) {
        click(vecino);
      }
    }
...
function actualizarContadorMinas() {
  contadorDiferencia.textContent =
    "Diferencias de minas y banderas plantadas: " +
    (juego.minasTotales - juego.banderasPlantadas);
}
...
function mostrarTablaDePartidas() {
  obtenerPartidas().then(function (partidas) {
    if (partidas.length === 0) return;

    var tablaHTML = "<h3 style='text-align:center'>Ranking de Partidas</h3>";
    tablaHTML += "<table>";
    tablaHTML +=
      "<thead><tr><th>Nombre</th><th>Fecha</th><th>Hora</th><th>Duración</th><th>Puntaje</th></tr></thead><tbody>";
    ...
    contenidoRanking.innerHTML = tablaHTML;
  });
}
...
// Ranking Modal
// ==========================
activarRanking.addEventListener("click", function () {
